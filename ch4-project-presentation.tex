...
Yes, those fancy QR codes are nothing more than just a ``link'', which (hopefully) follows the Key Uri Format specified by Google\cite{key-uri-format}.
...
Initially, the only way to enter a TOTP secret into \app's database was to enter the secret and the name manually.
But, what if the app only shows a QR, and does not provide a plaintext equivalent?
In that case, the user could use an online tool to decode the QR image.
However, this is a questionable solution, since it involves blind trust in a random website which could easily retain/ forward the decoded string.
...
You may think, why isn't \app{} deleting the image after decoding?
Well, that would break the philosophy of \app{}: it shall not manipulate storage outside the USB stick.
Also, the user might not expect this behavior.
...
The image is opened with the \t{pillow} library, and the \t{zxingcpp.read\_barcodes(image)} call gives the decoded text.
...
This is to follow the guidelines published by NIST SP 800-63B-4, Section 3.1.1, which says:
``Verifiers and CSPs SHALL require passwords that are used as a single-factor authentication mechanism to be a minimum of 15 characters in length.''\cite{nist-password-length}
...
Nowadays, they are rather an obstacle, in defining long and memorable passphrase.
...
There is also a hard limit of 100 maximum characters for the password, but this is just a sanity check.


\subsubsection{TPM and the generation of the TPM secret.} \label{lab:ch4-tpm}

\app{} currently supports TPM functionalities only on Linux.
...
For the virtualization of the TPM, the \t{swtpm} Linux library was used.
...
You can note the slight differences between Listing \ref{default-fapi-config} and Listing \ref{virtual-fapi-config}: there is a port defined, and a different profile is used.
...
If configuration, provisioning, and all the setup steps are done correctly and successfully, the actual usage of the TPM through FAPI happens through just a few lines of code.
...
If the result contains the secret path, it means the secret was generated.
...
This is because \t{tpm2-pytss} is a wrapper for the \t{tpm2-tss} library written in C, and probably the wrapper's authors could not combat logging.
...
So instead of balancing between security and usability with random trial and error, I followed trusted guidelines.
...
This approach avoids the common pitfall of using the same nonce twice, which often turns such algorithms ``from gold-standard to broken'' (see \cite{aes-gcm} for an article which describes AES-GCM, which is another variant but works similarly to AES-EAX).
...
The \t{set\_key} method solves this exact issue: the key (internal state) is changed, but the instance (the reference) remains the same, so the change is ``implicitly propagated'' throughout the code base.
...
Although these technologies are usually associated with web applications, \app{} is completely offline.
...
For example, it calculates the remaining time until TOTP code expiration, since it is more efficient to do this on the front-end, and periodically calls the backend for retrieving new TOTP codes.
...
The most important point is that this file contains an event listener, which catches when the view is loaded (\t{pywebviewready} envent).
After that event, the JS API is called, and execution gets under control.
From that point on, depending on user actions, the views will be changed.
...
This time, if enter or the \t{Unlock} button is pressed, the password is verified against the old password's hash.
...
To add manually, the user has to enter the TOTP secret into the secret field, and also give it a unique name in the name field, after which, the \i{Add} button shall be clicked.
...
In this case, the user has to press the QR button, which opens up the default file navigator, and lets the user choose a file with the extension \t{.bmp}, \t{.jpg}, \t{.jpeg}, \t{.png} or \t{.gif}.
...
As a result, there are four colors used throughout \app{}, which have the color codes \t{\#fffcfc}, \t{\#c4bce0}, \t{\#8892c6} and \t{\#373c61}.
They are defined as CSS constants \t{-{}-color-1}, \t{-{}-color-2}, \t{-{}-color-3} and \t{-{}-color-4}.
...
For the alignment of the elements, I used display flex on \t{div}s.
The body's, through the \t{display:flex, flex-direction:column, min-height:100\%} CSS attributes, enables this.
The body occupies the whole available space, and from that point on, we think of the UI as stacking rows on each other.
...
I put in the \t{res} folder the \t{.tff} files for regular, bold and italic, although currently only the regular variant is used (there is no italic or bold text in \app{}).
This font looks well, but most importantly, is monospaced, meaning that each character has the same length.
I like the monospaced property since with it things align better.
A lot of monospaced fonts look ugly on a UI, giving code editor/ terminal vibes, but this font looks fairly friendly.
And most importantly, it is rounded and clearly understandable.
...
Since \app{} runs from a USB stick, specifying one such path statically is impossible.
So, the Linux pack ships without an icon, but the icon is included in the resources.
If the user has the will and the necessary technical skills, he/ she can define a \t{.desktop} file using it.
...
These are defined as fields of \t{MasterController}: JS can call them as shown in Table \ref{tab:api}.
...
For some reason, this was not a problem on Linux (and the development machine), but it was a problem on Windows (one of the testing VMs).
The solution was to prepend the name of the class field with an underscore (\t{window} $rightarrow$ \t{\_window}).
This is a common convention in Python to mark private fields; pywebview then knows that I do not wish to expose to JS this field, and ignores it.
...
In the following, I present you a full cycle of what happens when the user clicks \i{Enter} on the register view (see: Figure \ref{fig:register-view}).
...
The Python handler has the same name as the JS handler, but following the underscore naming convention of Python.

Then, there are the data type conventions.
The classes of model do not follow a strict guideline: some of them return \t{True}/ \t{False} on success/ error, some of them return \t{None} on failure, and some of them do not need any king of failure handling.
...
\subsection{General Considerations} \label{lab:ch4-general-considerations}
...
These are marked in the requirements file with ``flags'' like: \t{sys\_platform == "linux"}, and ignored on other platforms.
...
A virtual environment (\t{venv}) is used to isolate Python packages used by \app{} from other Python packages on the system.
This is standard dependency management, and is required not only for ensuring that \app{} works well, but also to avoid breaking other apps.
Each developer creates his/ her own virtual environment and these are not pushed to git (specified in \t{.gitignore}).
...
However, it is not a cross-compiler; to make a Windows app you run PyInstaller on Windows, and to make a Linux app you run it on Linux, etc.''\footnote{\url{https://pyinstaller.org/en/stable/}}

The commands themselves differ on Linux and on Windows as well.
Paths work differently: Linux uses /, Windows uses $\backslash$.
Application icons are treated differently: Windows needs a \t{.ico} file, Linux has no such concept.
So, there are different setups, commands for both OSes, although they have the same intent.
...
Listing \ref{lst:linux-setup} installs some Linux packages which are dependencies for the Python packages listed in \t{requirements.txt}.
...
\paragraph{Real TPM}
Follow these steps if you wish to run \app{} with a real, physical TPM.
...
These affect only FAPI, not the whole TPM (I have disk encryption which uses TPM, and this command did not break it).
...
\subsubsection{Setup} \label{lab:ch4-setup-on-windows}
...
Command \t{where python} shall output as first entry \t{$\backslash$venv$\backslash$Scripts$\backslash$python.exe}; if not, try activating again/ in a new terminal.
...
\subsubsection{Packing} \label{lab:ch4-packing-windows}
...
If you want to create a release/archive, navigate to the previous command's output (\t{C:$\backslash$Users$\backslash$YOUR-USERNAME$\backslash$AppData$\backslash$Local$\backslash$Temp$\backslash$authenstickator$\backslash$dist} by default), and pack it using \t{tar}.
See Listing \ref{lst:archive-linux} for an example of packing for an Ubuntu release, and later extracting.
...
\begin{figure}[htbp]
  \makebox[\textwidth][c]{\includegraphics[width=1.3\textwidth]{img/usb-windows.png}}
...
Otherwise, execution fails silently/ with an unrelated error message, because Windows.
If you have the \t{\_internal} folder and the \t{\app{}} executable, you are ready to go!
